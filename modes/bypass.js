...
class BypassMode {
    ...
    async handle(request, sessionId) {
        ...
        return await this.fetchDirect(targetUrl, request, sessionId);
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = client.request(options, (res) => {
                ...
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                    ...
                    res.resume();
                    resolve({ status: res.statusCode, headers: responseHeaders, body: Buffer.alloc(0) });
                    return;
                }
                ...
                res.on('end', () => {
                    ...
                    if (contentType.includes('text/html') || contentType.includes('text/css') || contentType.includes('javascript')) {
                        ...
                        body = Buffer.from(this.rewriter.rewrite(text, targetUrl, contentType), 'utf-8');
                    }
                    
                    responseHeaders['content-length'] = body.length;
                    
                    resolve({ status: res.statusCode, headers: responseHeaders, body: body });
                });
            });
            ...
            if (request.body) {
                let bodyData;
                if (typeof request.body === 'string') bodyData = Buffer.from(request.body);
                else if (Buffer.isBuffer(request.body)) bodyData = request.body;
                else bodyData = Buffer.from(JSON.stringify(request.body));
    ...
    cleanHeaders(headers, parsed) {
        const clean = {};
        const skip = ['content-security-policy','x-frame-options','strict-transport-security','content-length','transfer-encoding','connection','keep-alive','upgrade','content-encoding'];
        
        for (const key of Object.keys(headers || {})) {
            const lower = key.toLowerCase();
            if (!skip.includes(lower)) clean[key] = headers[key];
        }
